...
const Mapa = () => {
  ...
  useEffect(() => {
    navigator.geolocation.getCurrentPosition(async (position) => {
      ...
      RutaDirecciones.push(await getDirections(current, LugaresCercanos[0]));
      RutaDirecciones.push(await getDirections(current, LugaresCercanos[1]));
      RutaDirecciones.push(await getDirections(current, LugaresCercanos[2]));
      RutaDirecciones.push(await getDirections(current, LugaresCercanos[3]));
      RutaDirecciones.push(await getDirections(current, LugaresCercanos[4]));
    ...
    do {
      const closest = lugares
        .map((lugar) => {
          ...
          if (
            coord.lat !== lugarInicial.lat &&
            coord.lon !== lugarInicial.lon
          ) {
            return {
              coord,
              dist: geolib.getDistance(lugarInicial, coord),
            };
          }
        })
        .sort((a, b) => a.dist - b.dist)[LugaresCernanos.length];

      LugaresCernanos.push(closest.coord);
    } while (LugaresCernanos.length <= 5);
